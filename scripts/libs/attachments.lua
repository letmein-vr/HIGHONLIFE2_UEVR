...
-- HOL2 PERF FIX: Pre-cache the MuzzleSocket FName once at call-time (lazy init).
-- fname_from_string() allocates a new heap object every call via Conv_StringToName.
-- At 60fps that was ~3,600 dead objects/min → periodic Lua GC spikes every ~2 seconds.
-- The FName is interned permanently in UE's name table so reusing it is safe forever.
local _muzzleSocketFName = nil
...
function M.broadcastGrippedAttachmentRotation()
	...
	-- HOL2 PERF FIX: Early-exit when nothing is gripped — skips both
	-- getCurrentGrippedAttachment pairs() walks AND the GetSocketRotation bridge call.
	if leftAttachment == nil and rightAttachment == nil then
		return
	end

	-- Lazily create the cached FName on the first frame a weapon is held.
	if _muzzleSocketFName == nil then
		_muzzleSocketFName = uevrUtils.fname_from_string("MuzzleSocket")
	end

	local leftRot  = (leftAttachment  and leftAttachment.GetSocketRotation  ~= nil)
	                 and leftAttachment:GetSocketRotation(_muzzleSocketFName)  or nil
	local rightRot = (rightAttachment and rightAttachment.GetSocketRotation ~= nil)
	                 and rightAttachment:GetSocketRotation(_muzzleSocketFName) or nil

	executeGripAttachmentRotationChange(leftRot, rightRot)
