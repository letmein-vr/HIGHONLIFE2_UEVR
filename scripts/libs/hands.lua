...
local function getComponentName(componentName, hand)
	...
	if componentName == nil then --just get the first one
		for name, component in pairs(handComponents) do
			if hand == nil or component[hand] ~= nil then
				foundName = name
				break
			end
		end
...
function M.getHandComponent(hand, componentName)
	componentName = getComponentName(componentName, hand)
...
local function getAttachmentStateAndExtension(attachment)
	...
	elseif type(attachment) == "string" and attachment ~= "attachment_none" then
		...
		if attachment ~= "" and attachment ~= "Default" then
	...
	if configuration ~= nil and configuration["profiles"] ~= nil then
		...
		if profile ~= nil then
			...
			for key, mesh in pairs(profile) do
				local rightMeshProp = mesh["Right"] ~= nil and mesh["Right"]["Mesh"] or ""
				local leftMeshProp  = mesh["Left"] ~= nil and mesh["Left"]["Mesh"] or ""
				local sharedMeshProp= mesh["Mesh"] or ""

				if rightMeshProp == "" and leftMeshProp == "" then
					rightMeshProp = sharedMeshProp
					leftMeshProp = sharedMeshProp
				else
					if rightMeshProp == "" then rightMeshProp = leftMeshProp end
					if leftMeshProp == "" then leftMeshProp = rightMeshProp end
				end

				local function getMeshFn(propName)
					if propName == "" then return nil end
					if propName == "Custom" then
						if getCustomHandComponent ~= nil then return getCustomHandComponent(key) end
					else
						return uevrUtils.getObjectFromDescriptor(propName, false)
					end
					return nil
				end

				local leftComponent = getMeshFn(leftMeshProp)
				local rightComponent = getMeshFn(rightMeshProp)

				if leftComponent ~= nil or rightComponent ~= nil then
					if leftComponent == rightComponent then
						components[key] = leftComponent
					else
						components[key] = { left_mesh = leftComponent, right_mesh = rightComponent }
					end
				end
			end
...
function M.create(skeletalMeshComponent, definition, handAnimations)
	if definition ~= nil then
		for name, skeletalMeshDefinition in pairs(definition) do
			...
			local isWrapper = type(component) == "table" and (component.left_mesh ~= nil or component.right_mesh ~= nil)
			if isWrapper or uevrUtils.validate_object(component) ~= nil then
				handComponents[name] = {}
				for index = Handed.Left , Handed.Right do
					local actualComponent = component
					if isWrapper then
						actualComponent = (index == Handed.Left) and component.left_mesh or component.right_mesh
					end
					if uevrUtils.validate_object(actualComponent) ~= nil then
						handComponents[name][index] = M.createComponent(actualComponent, name, index, skeletalMeshDefinition[index==Handed.Left and "Left" or "Right"], skeletalMeshDefinition)
					end
...
function M.createComponent(skeletalMeshComponent, name, hand, definition, rootDefinition)
	...
	if uevrUtils.validate_object(skeletalMeshComponent) ~= nil then
		...
		if component ~= nil then
			...
			local myOffset = {X=0,Y=0,Z=0,Pitch=0,Yaw=0,Roll=0}
			if definition ~= nil and definition["Offset"] ~= nil then
				myOffset = definition["Offset"]
			elseif rootDefinition ~= nil and rootDefinition["Offset"] ~= nil then
				myOffset = rootDefinition["Offset"]
			end
			
			local myIgnoreRelative = false
			if definition ~= nil and definition["IgnoreRelativeOffset"] ~= nil then
				myIgnoreRelative = definition["IgnoreRelativeOffset"] == true
			elseif rootDefinition ~= nil and rootDefinition["IgnoreRelativeOffset"] ~= nil then
				myIgnoreRelative = rootDefinition["IgnoreRelativeOffset"] == true
			end

			local baseRotation = myIgnoreRelative and uevrUtils.rotator(0,0,0) or skeletalMeshComponent.RelativeRotation
			uevrUtils.set_component_relative_transform(component, myOffset, {Pitch=baseRotation.Pitch+myOffset.Pitch, Yaw=baseRotation.Yaw+myOffset.Yaw,Roll=baseRotation.Roll+myOffset.Roll})
...
--only for debugging
function M.updateOffset(skeletalMeshComponent, newOffset, ignoreRelative, hand, componentName)
	if skeletalMeshComponent ~= nil then
		if hand == nil then hand = Handed.Right end
		if componentName == nil then componentName = "Arms" end
		local baseRotation = ignoreRelative and uevrUtils.rotator(0,0,0) or skeletalMeshComponent.RelativeRotation
		local component = handComponents[componentName] and handComponents[componentName][hand]
		if component ~= nil then
...
function M.handleInputForHands(state, rightAttachment, leftAttachment, overrideTrigger, allowAutoHandle)
	...
	for id, target in pairs(inputHandlerAnimID) do
		for hand = Handed.Left, Handed.Right do
			...
			if not isHoldingAttachment then
				local isButtonPressedGrip = uevrUtils.isButtonPressed(state, hand == Handed.Right and XINPUT_GAMEPAD_RIGHT_SHOULDER or XINPUT_GAMEPAD_LEFT_SHOULDER)
				animation.updateAnimation(handStr.."_"..target, handStr.."_grip_unarmed", isButtonPressedGrip, {duration=animDuration})
				...
				local isButtonPressedTrigger = weaponHandTriggerValue > 100
				animation.updateAnimation(handStr.."_"..target, handStr.."_trigger_unarmed", isButtonPressedTrigger, {duration=animDuration})
				...
				local isButtonPressedThumb = uevr.params.vr.is_action_active(weaponhandRest, weaponhandController)
				animation.updateAnimation(handStr.."_"..target, handStr.."_thumb", isButtonPressedThumb, {duration=animDuration})
			else
				local isButtonPressedGrip = uevrUtils.isButtonPressed(state, hand == Handed.Right and XINPUT_GAMEPAD_RIGHT_SHOULDER or XINPUT_GAMEPAD_LEFT_SHOULDER)
				animation.updateAnimation(handStr.."_"..target, handStr.."_grip_weapon"..attachmentExtension, isButtonPressedGrip, {duration=animDuration})
				
				local weaponHandTriggerValue = (hand == Handed.Right or overrideTrigger) and state.Gamepad.bRightTrigger or state.Gamepad.bLeftTrigger
				local isButtonPressedTrigger = weaponHandTriggerValue > 100
				animation.updateAnimation(handStr.."_"..target, handStr.."_trigger_weapon"..attachmentExtension, isButtonPressedTrigger, {duration=animDuration})
...
function M.debug(skeletalMeshComponent, hand, rightTargetJoint, disableVisualization)
	...
	if uevrUtils.validate_object(skeletalMeshComponent) ~= nil then
		...
		if handComponents["Arms"] == nil then handComponents["Arms"] = {} end
